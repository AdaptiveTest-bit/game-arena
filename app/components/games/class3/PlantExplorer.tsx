...
import { usePlantExplorerStore } from '@/app/store/usePlantExplorerStore';
import { submitTelemetry } from '@/app/utils/gameUtils';
import { 
  RotateCcw, Trophy, ChevronRight, 
  Lightbulb, ArrowRight, Star, Flower,
  TreeDeciduous, Sprout, Leaf
} from 'lucide-react';
...
const PlantExplorer: React.FC = () => {
  ...
  const getPlantIcon = (type: string) => {
    switch (type) {
      case 'tree': return <TreeDeciduous size={24} />;
      case 'shrub': return <Flower size={24} />;
      case 'herb': return <Sprout size={24} />;
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-600 via-emerald-600 to-teal-500 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-5xl font-bold text-white mb-4">Plant Explorer</h1>
            <p className="text-xl text-gray-100">
              Discover Trees, Shrubs, Herbs & More! 🌳
            </p>
          </div>
          ...
          <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-100">
              <div className="flex items-start gap-3">
                ...
                <p>Identify plant types: Trees, Shrubs, Herbs</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🍎</span>
                <p>Match plants to their uses: Food, Medicine, Wood</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🌱</span>
                <p>Learn about plant parts and functions</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🔬</span>
                <p>Understand photosynthesis basics</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              { level: 1, icon: '🌱', color: 'green', desc: 'Basic plant types' },
              { level: 2, icon: '🌳', color: 'yellow', desc: 'Plant uses & classification' },
          ...
          {currentQuestion && (
            <>
              ...
              {currentQuestion.type === 'classification' && (
                <div className="space-y-3">
                  <p className="text-gray-600 mb-2">Click the correct type for each item:</p>
                  {currentQuestion.options?.map((option, idx) => {
                    const isSelected = selectedAnswer === option;
                    ...
                    return (
                      ...
                      >
                        <span className="text-2xl">
                          {option.toLowerCase().includes('tree') ? '🌳' : 
                           option.toLowerCase().includes('shrub') ? '🌹' :
                           option.toLowerCase().includes('herb') ? '🌱' :
                           option.toLowerCase().includes('creeper') ? '🎃' : '❓'}
                        </span>
                        <span className="flex-1 font-medium">{option}</span>
                        <span className="text-sm text-gray-500">Click to answer</span>
                      </button>
                    );
                  })}
                </div>
