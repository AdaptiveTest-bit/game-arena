...
import { useHelperMatchStore } from '@/app/store/useHelperMatchStore';
import { submitTelemetry } from '@/app/utils/gameUtils';
import { 
  RotateCcw, Trophy, ChevronRight, 
  Lightbulb, ArrowRight, Star, Heart,
  Stethoscope, BookOpen, Shield, Phone,
  Truck, Mail, Scissors, ChefHat
} from 'lucide-react';

const HelperMatch: React.FC = () => {
  const {
    startGame,
    selectAnswer,
    nextQuestion,
  ...
  const getHelperIcon = (helper: string) => {
    ...
    if (lower.includes('doctor') || lower.includes('stethoscope')) return <Stethoscope size={24} />;
    if (lower.includes('teacher') || lower.includes('book')) return <BookOpen size={24} />;
    if (lower.includes('police') || lower.includes('shield')) return <Shield size={24} />;
    if (lower.includes('fire')) return <Heart size={24} />;
    if (lower.includes('driver') || lower.includes('truck')) return <Truck size={24} />;
    if (lower.includes('postman') || lower.includes('mail')) return <Mail size={24} />;
    if (lower.includes('barber') || lower.includes('scissor')) return <Scissors size={24} />;
    if (lower.includes('baker') || lower.includes('chef')) return <ChefHat size={24} />;
    return <Heart size={24} />;
  };

  const getHelperEmoji = (helper: string) => {
    const lower = helper.toLowerCase();
    if (lower.includes('doctor')) return '👨‍⚕️';
    if (lower.includes('teacher')) return '👩‍🏫';
    if (lower.includes('police')) return '👮';
    if (lower.includes('firefighter')) return '👨‍🚒';
    ...
    if (lower.includes('baker')) return '👨‍🍳';
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-500 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-5xl font-bold text-white mb-4">Helper Match</h1>
            <p className="text-xl text-gray-100">
              Match Helpers with Tools & Work! 🤝
            </p>
          </div>
          ...
          <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-100">
              <div className="flex items-start gap-3">
                ...
                <p>Identify helpers: Doctor, Teacher, Police, Firefighter</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🔧</span>
                <p>Match helpers with their tools</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">💼</span>
                <p>Understand the work each helper does</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">❤️</span>
                <p>Learn why helpers are important</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              { level: 1, icon: '🌟', color: 'green', desc: 'Basic helper identification' },
              { level: 2, icon: '🔧', color: 'yellow', desc: 'Tools & work matching' },
                ...
                <div className="space-y-3">
                  {currentQuestion.options.map((option, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-2xl">{getHelperEmoji(option)}</span>
              ...
              {showFeedback && (
                <div className="mt-6 space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-4 rounded-lg transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-2"
                  >
                    Next Question
                    <ArrowRight size={20} />
                  </button>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 border-2 border-blue-500">
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-indigo-50 p-4 rounded-lg text-center">
                ...
                <p className="text-3xl font-bold text-indigo-600">{correctAnswers}/10</p>
              </div>
            </div>
              ...
              {passedLevel ? (
                currentLevel < 3 ? (
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
                  >
