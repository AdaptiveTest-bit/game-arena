...
import { useAnimalKingdomStore } from '@/app/store/useAnimalKingdomStore';
import { submitTelemetry } from '@/app/utils/gameUtils';
import { 
  RotateCcw, Trophy, ChevronRight, 
  Lightbulb, ArrowRight, Star, PawPrint,
  Bird, Fish, Rabbit
} from 'lucide-react';

const AnimalKingdomGame: React.FC = () => {
  const {
    startGame,
    selectAnswer,
    nextQuestion,
  ...
  const getAnimalEmoji = (animal: string) => {
    const wildAnimals = ['lion', 'tiger', 'elephant', 'monkey', 'snake', 'deer', 'zebra', 'giraffe', 'bear', 'wolf'];
    const petAnimals = ['dog', 'cat', 'fish', 'parrot', 'rabbit', 'hamster', 'turtle', 'canary'];
    const farmAnimals = ['cow', 'buffalo', 'goat', 'sheep', 'horse', 'pig', 'chicken', 'duck', 'donkey'];
    
    const lowerAnimal = animal.toLowerCase();
    if (wildAnimals.includes(lowerAnimal)) return '🦁';
    if (lowerAnimal.includes('cat')) return '🐱';
    if (lowerAnimal.includes('dog')) return '🐶';
    if (lowerAnimal.includes('fish')) return '🐟';
    if (lowerAnimal.includes('bird') || lowerAnimal.includes('parrot') || lowerAnimal.includes('chicken') || lowerAnimal.includes('duck')) return '🐦';
    if (lowerAnimal.includes('rabbit') || lowerAnimal.includes('hare')) return '🐰';
    if (lowerAnimal.includes('cow') || lowerAnimal.includes('buffalo')) return '🐄';
    if (lowerAnimal.includes('horse')) return '🐴';
    if (lowerAnimal.includes('pig')) return '🐷';
    if (lowerAnimal.includes('goat') || lowerAnimal.includes('sheep')) return '🐐';
    if (lowerAnimal.includes('elephant')) return '🐘';
    if (lowerAnimal.includes('monkey')) return '🐒';
    if (lowerAnimal.includes('snake')) return '🐍';
    if (lowerAnimal.includes('deer')) return '🦌';
    return '🐾';
  };

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'wild': return <PawPrint size={20} />;
      case 'pet': return <Rabbit size={20} />;
      case 'farm': return <Fish size={20} />;
      default: return <Bird size={20} />;
    }
  };
  ...
  if (gameState === 'menu') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-600 via-amber-600 to-yellow-500 p-8">
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <div className="text-center mb-12">
            <div className="text-6xl mb-4">🦁</div>
            <h1 className="text-5xl font-bold text-white mb-4">Animal Kingdom Game</h1>
            <p className="text-xl text-gray-100">
              Classify Animals & Match to Food & Homes! 🐾
            </p>
          </div>
          ...
          <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-100">
              <div className="flex items-start gap-3">
                ...
                <p>Identify Wild Animals: Lion, Tiger, Monkey, Deer</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🐕</span>
                <p>Identify Pet Animals: Dog, Cat, Fish, Parrot</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🐄</span>
                <p>Identify Farm Animals: Cow, Chicken, Horse</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-2xl">🍖</span>
                <p>Match animals to their food and homes</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              { level: 1, icon: '🐾', color: 'green', desc: 'Basic classification' },
              { level: 2, icon: '🦁', color: 'yellow', desc: 'Food & homes matching' },
              { level: 3, icon: '🏆', color: 'red', desc: 'Food chains & adaptation' },
            ].map((level) => (
              <button
                key={level.level}
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {currentQuestion.category && (
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-100 text-orange-700">
                    ...
                    <span className="font-semibold capitalize">{currentQuestion.category}</span>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  {currentQuestion.options.map((option, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-2xl">{getAnimalEmoji(option)}</span>
              ...
              {currentQuestion.type === 'fill_blank' && (
                <div>
                  <input
                    ...
                    className="w-full p-4 text-lg border-2 border-gray-300 rounded-lg focus:border-orange-500 focus:outline-none"
                    maxLength={100}
                  />
                  {!answered && (
                    <button
              ...
              {showFeedback && (
                <div className="mt-6 space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold py-4 rounded-lg transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-2"
                  >
                    Next Question
                    <ArrowRight size={20} />
                  </button>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 border-2 border-orange-500">
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-orange-50 p-4 rounded-lg text-center">
                ...
                <p className="text-3xl font-bold text-orange-600">{correctAnswers}/10</p>
              </div>
            </div>
              ...
              {passedLevel ? (
                currentLevel < 3 ? (
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
                  >
