...
import { generateOperationQuestions, OMQuestion } from '@/app/utils/operationMasterGenerator';

const OperationMasterArena: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
  const [questions, setQuestions] = useState<OMQuestion[]>([]);
  ...
  useEffect(() => {
    const qs = generateOperationQuestions(6);
    setQuestions(qs);
  }, []);
  ...
  const handleRestart = () => {
    ...
    setSelected(null);
    setShowAnswer(false);
    setQuestions(generateOperationQuestions(6));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-indigo-100 p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-6">
        <h1 className="text-2xl font-bold mb-2">Operation Master Arena</h1>
        <p className="text-sm text-gray-600 mb-4">Chapters: Addition, Subtraction, Word Problems, Mental Math</p>
        ...
        {showAnswer && !showResult && (
          <div className="mt-4 p-4 bg-gray-100 rounded">
            ...
            <p className="text-sm text-gray-700">Correct answer: {current.answer}.</p>
          </div>
        )}
...
export default OperationMasterArena;
