...
interface SubtractionSnakeCanvasProps {
  width: number;
  height: number;
}

const DIFFICULTY_COLORS = {
  easy: { primary: '#10b981', secondary: '#059669', bg: '#d1fae5', border: '#047857' },
...
const SubtractionSnakeCanvas: React.FC<SubtractionSnakeCanvasProps> = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    answers.forEach((answer) => {
      ...
      const hue = (answer.value * 47 + 180) % 360;
    ...
    if (gameOver) {
      ...
      ctx.fillText('Keep practicing subtraction!', width / 2, height / 2 + 60);
    }
    ...
    if (!gameStarted && !gameOver) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('🐍 SUBTRACTION SNAKE ➖', width / 2, height / 2 - 80);
      
      ctx.font = '22px Arial';
      ctx.fillText('Master Subtraction from 1 to 100!', width / 2, height / 2 - 30);
...
export default SubtractionSnakeCanvas;
